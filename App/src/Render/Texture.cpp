...
namespace Render {
	...
	public:
		...
		[[nodiscard]] GLenum GetInternalFormat() const
		{
			...
			CORE_WARNING("No valid internal format found. Defaulting to GL_R8.");
			return GL_R8;
		}
		...
		[[nodiscard]] GLenum GetFormat() const
		{
			...
			CORE_ASSERT(false, "The pixel format {0} do not exist...", (int)spec.pixelType);
			return GL_RGBA;
		}
		...
		[[nodiscard]] GLenum GetType() const
		{
			...
			CORE_ASSERT(false, "The pixel type {0} do not exist...", (int)spec.pixelType);
		...
		[[nodiscard]] GLint GetMagFilter() const
		{
			...
			CORE_WARNING("No value found for MagFilter, defaulting to 'Linear'.");
			return GL_LINEAR;
		}
		...
		[[nodiscard]] GLint GetMinFilter() const
		{
			...
			CORE_WARNING("No value found for MinFilter, defaulting to 'Linear'.");
			return GL_LINEAR;
		}
		...
		[[nodiscard]] GLint GetWrapS() const
		{
			...
			CORE_WARNING("No value found for WrapS, defaulting to 'Repeat'.");
			return GL_REPEAT;
		}
		...
		[[nodiscard]] GLint GetWrapT() const
		{
			...
			CORE_WARNING("No value found for WrapT, defaulting to 'Repeat'.");
			return GL_REPEAT;
		}
	};


	Core::Ref<Texture> Texture::Create(Texture2DSpecification spec, Core::Buffer image)
	{
		return CreateRef<Texture>(spec, image);
	}

	Core::Ref<Texture> Texture::Create(Texture2DSpecification spec)
	{
	...
	void Texture::CreateTexture() {
		...
		CORE_ASSERT(m_Spec.channels > 0 && m_Spec.channels < 5, "The number of channel {0} is not handle at the moment.", m_Spec.channels);
	...
	void Texture::SetData(Core::Buffer image)
	{
		TexSpecHelper helper(m_Spec);
		GLenum dataFormat = helper.GetFormat();
		GLenum pixelType = helper.GetType();
		CORE_ASSERT(image.data != nullptr, "No data where found on the image.");
		CORE_ASSERT(m_Spec.size() * helper.GetPixelSize() == image.size, "The size of the image ({}) is different from the information of the texture of size {} (width: {}, height: {}, channel: {}, pixelType: '{}')", image.size, m_Spec.size(), m_Spec.width, m_Spec.height, m_Spec.channels, helper.GetTypeToString());
