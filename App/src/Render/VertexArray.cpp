...
namespace Render {
   GLenum BufferDataTypeToOpenGLBaseType(BufferDataType type)
   {
      ...
      CORE_ASSERT(false, "Unknown ShaderDataType {0}.", (int)type);
      return 0;
   }

   Core::Ref<VertexArray> VertexArray::Create() {
      return Core::CreateRef<VertexArray>();
   }
   ...
   std::optional<uint32_t > VertexArray::AddVertexBuffer(Core::Ref<VertexBuffer> vb) {
      CORE_PROFILE_FUNCTION();
   ...
   bool VertexArray::SetVertexBuffer(uint32_t vbIndex, Core::Ref<VertexBuffer> vb) {
      CORE_PROFILE_FUNCTION();
      if(!vb) return false;
   ...
   void VertexArray::SetIndexBuffer(Core::Ref<IndexBuffer> vb) {
      CORE_PROFILE_FUNCTION();
